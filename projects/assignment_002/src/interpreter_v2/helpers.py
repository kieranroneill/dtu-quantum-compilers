...
def node_name(t):
    ...
    if type(t) == Token:
        return t.value if t.type == "RULE" else t.type
    if type(t) == Tree:
        return t.data if type(t.data) == str else t.data.value
...
def numerical_type(v):
    if type(v) == bool or type(v) == np.bool:
        return "cbit"
    if type(v) == int or type(v) == np.int64:
        return "int"
...
def prune_tree(tree):
    if isinstance(tree, Token):
        return tree
    if node_name(tree) == "exp":
        return prune_exp(tree)
    else:
...
evaluate_unop = {"-": lambda x: -x, "~": lambda x: ~x, "not": lambda x: (not x)}
...
def make_procedure(name, parameters, statement):
    return Tree(
        Token("RULE", "procedure"), [name, Tree(Token("RULE", "parameter_declarations"), parameters), statement]
    )
